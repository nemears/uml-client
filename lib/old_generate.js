import { extendBaseElement, randomID, TypeInfo } from './types/element.js';
import Singleton from './singleton.js';
import UmlSet from './set.js';
import { KERNEL_TYPES } from './modelIds.js';
import { restoreReferences } from './abstractUmlClient.js';
...
export async function generate(element, umlClient, parentData) {
    ...
    if (element.is('Classifier')) {
        ...
        const classConstructor = (me, manager) => {
            ...
            me.delete = async function() {
                for (const referencePair of me.references) {
                    ...
                    referencedElement.references.delete(me.id);
                    try {
        ...
        ret[element.name + 'Manager'] = new Manager();

        const created_manager = ret[element.name + 'Manager'];

        // TODO populate create_manager._types
        ...
        ret[element.name + 'Manager'] = function() {
            this.id = randomID();
            this.graph = new Map();
            this.updateHandlers = [];
            ...
            this.apply = async function(el, stereotype) {
                ...
                await this.get(element); // update el

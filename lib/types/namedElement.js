...
import { DEPENDENCY_CLIENTS_ID, NAMED_ELEMENT_NAMESPACE_ID, NAMED_ELEMENT_CLIENT_DEPENDENCIES_ID, NAMESPACE_OWNED_MEMBERS_ID } from '../modelIds';

export function assignNamedElementSets(namedEl) {
    const base = namedEl.typeInfo;
    namedEl.typeInfo = new TypeInfo('NamedElement');
    namedEl.typeInfo.setBase(base);
    namedEl.typeInfo.specialEmit = (data) => {
        if (name !== '') {
            data.name = namedEl.name;
        }
    };
    namedEl.name = ""; 

export function oppositeAdd(set, el) {
    if (set.opposite !== undefined) {
        const opposite = el.sets.get(set.opposite);
...
export default class UmlSet {
    ...
    add(el) {
        ...
        if (typeof el !== 'string') {
            ...
            // handle policy
            if (this.additionPolicy) {
                this.additionPolicy(el, this.el);
            }
    ...
    remove (el) {
        ...
        const removeFromSet = (set) => {
            ...
            for (let redefinedSet of set.redefinedSets) {
                if (redefinedSet.contains(el)) {
                    removeFromSet(redefinedSet);
                }
            }

            if (set.removalPolicy) {
                set.removalPolicy(el, this.el);
            }
        }

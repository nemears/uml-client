...
export async function restoreReference(elementReference, el) {
    let elFromClient = el.manager.getLocal(elementReference.id);
    if (elFromClient === undefined) {
        elFromClient = await el.manager.get(elementReference.id);
    }
    if (elFromClient === undefined) {
        throw new Error("could not find element in set with id " + elementReference.id);
    }
    elementReference.el = elFromClient;
    const reference = elFromClient.references.find((reference) => reference.id === el.id);
    if (!reference) {
        elFromClient.references.push(new ElementReference(el));
    } // TODO increase count
...
export default class Set {
    ...
    add(el) {
        ...
        if (typeof el !== 'string') {
            // handle references
            let reference = el.references.find((reference) => reference.id == this.el.id);
            if (!reference) {
                el.references.push(new ElementReference(this.el));
    ...
    remove (el) {
        ...
        this.data = this.data.filter(entry => el.id !== entry.id);
    ...
    async front() {
        ...
        if (this.data[0].el === null) {
            await restoreReference(this.data[0], this.el);
        }
            ...
            next: async () => {
                if (index < me.data.length) {
                    if (me.data[index].el === null) {
                        await restoreReference(me.data[index], me.el); 
                    }

...
import { subsetContains } from "./set";
...
export function emitAssociation(data, alias, association) {
    ...
    if (association.ownedEnds.size() > 0) {
        let emitSet = false;
        for (let id of association.ownedEnds) {
    ...
    if (association.memberEnds.size() > 0) {
        let emitSet = false;
        for (let id of association.memberEnds) {

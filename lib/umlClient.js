...
import { cleanupReferences, randomID } from "./types/element";
...
export default class UmlWebClient {
                        ...
                        me.client.onmessage = (msg) => {
                            ...
                            if (msgData.put || msgData.PUT) {
                                ...
                                const doLater = async () => {
                                    ...
                                    let parseData = await this.parse(elData);
    ...
    */
    ...
    post(type, options) {
        ...
        const camelToSnakeCase = str => str = str.replace(/[A-Z]/g, letter => `_${letter.toLowerCase()}`).substring(1);
        this.client.send(JSON.stringify({
    ...
    getFromServer(id) {
        ...
        return new Promise((resolveGetRequest) => {
            ...
            this._requestQueue.push(async (data) => {
                let elFromServer = await parse(data, this);
    ...
    async parse(elData) {
        const newElement = await parse(elData, this);

    ...
    */
    ...
    handleMessage(message) {
        ...
        if (data['put'] || data['PUT']) {
            ...
            // references
            if (oldElement) {
                for (let reference of oldElement.references) {
                    for (let setEntry of Object.entries(reference.el.sets)) {
                        if (setEntry[1].contains(elID)) {
                            setEntry[1].data.find((setReference) => setReference.id === elID).el = elFromServer;
                        }
                    }
                    const theirReference = reference.el.references.find((otherReference) => otherReference.id === elID);
                    if (theirReference) {
                        theirReference.el = elFromServer;
                    }
                    elFromServer.references.push(new ElementReference(reference.el));
                }
            }

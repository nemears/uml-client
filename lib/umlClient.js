...
export default class UmlWebClient {
    ...
    constructor(options) {
        this.initializationPromise = new Promise((res, rej) => {
            const address = options.address ? options.address : 'ws://localhost:1672';
            ...
            let me = this;
	        // let heartBeatMessages = undefined;

            const openConnection = () => {
                // log('client ' + me.id + ' opened connection with websocket server');
        ...
        this.eventEmitter.on('newRequest', async (request) => {
            if (!this.initialized) {
                // log('awaiting initialization for client ' + this.id);

...
export default class UmlWebClient {
    ...
    login(user, password, options) {
        ...
        this.initializationPromise = new Promise((res, rej) => {
            const address = this.options.address ? this.options.address : 'ws://localhost:1672';
            ...
            const openConnection = async () => {
                me.client.send(JSON.stringify({
                    ...
                    passwordHash: password === undefined ? '' : Array.from(new Uint8Array(await crypto.subtle.digest('SHA-256', new TextEncoder().encode(password)))).map(b => b.toString(16).padStart(2, '0')).join('')
                }));

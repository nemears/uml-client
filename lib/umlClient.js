...
export default class UmlWebClient {
    ...
    onClient = (clientID) => {
        console.warn('client of id ' + clientID + ' connectected to server. Override UmlWebClient.onClient to keep track of new clients!');
    }
        ...
        this.initialization = new Promise((resolveInitialization, rejectInitialization) => {
            ...
            me.client.onopen = async () => {
                ...
                if (options.password && options.password !== '') {
                    passwordHash = Array.from(new Uint8Array(await crypto.subtle.digest('SHA-256', new TextEncoder().encode(options.password)))).map(b => b.toString(16).padStart(2, '0')).join('');
                }
                    ...
                    if (msgObj.client === me.id) {
                        ...
                        me.client.onmessage = (msg) => {
                            ...
                            if (msgData.put || msgData.PUT) {
                                ...
                                const oldElement = me.#graph.get(elID);
                                let elFromServer = parse(elData);
                                me.#graph.set(elFromServer.id, elFromServer);
                                elFromServer.manager = me;
                                console.log('server ' + elFromServer.id);
    ...
    getProjectConfig() {
        ...
        return new Promise((res, rej) => {
            ...
            this.#requestQueue.push((msgData) => {
                if (msgData.project) {
                    res(msgData);
                } else {
                    rej(msgData);
    ...
    */
    ...
    getFromServer(id) {
        ...
        return new Promise((resolveGetRequest, rejectGetRequest) => {
            // send request for element to server
            this.client.send(JSON.stringify({
                get: id
            }));
            this.#requestQueue.push(async (data) => {
                let elFromServer = parse(data);
                if (elFromServer === undefined) {
                    rejectGetRequest('bad data!');
                } else {
                    this.#graph.set(elFromServer.id, elFromServer);
                    if (id === '') {

    ...
    if (element.is('Classifier')) {
        ...
        const classConstructor = (me) => {
            ...
            me.delete = async function() {
                for (const referencePair of me.references) {
                    for (const setPair of referencePair[1].sets) {
                        const set = setPair[1];
        ...
        ret[element.name + 'Manager'] = function(apiLocation) {
            ...
            umlClient.updateHandlers.push(async function(newElement, oldElement) {
                if (newElement) {
                    if (newElement.is('InstanceSpecification') && newElement.owner.id() === apiLocation.id) {
                    ...
                    // todo run delete
                    if (oldElement.is('InstanceSpecification') && oldElement.owner.id() === apiLocation.id) {
                    ...
                    while (queue.length > 0) {
                        ...
                        for await (const property of front.attributes) {
                            ...
                            if (multiplicity.lowerValue === 0 && multiplicity.upperValue === 1) {
                                ...
                                for (const instanceID of elValue.ids()) {

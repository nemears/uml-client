...
const KERNEL_TYPES = new Set([
    'XI35viryLd5YduwnSbWpxSs3npcu', // Element
    'Ox30_bFSsGtPZorPYOkDTLeXquIN', // NamedElement
    'zvAhfu&WNCI1cNXd3BkqhxywHAYY', // Package
    '&ve0WKXZ2Hn47xMl&Dl&lXPTMHD9', // PackageableElement
    'pUg_kaODi6D0KMi6MXAJcMWMG3G_', // Namespace
    'OM2kyIbDJFpRNvNRhSNDTl_xetVp'
]);
...
export async function generate(element, umlClient, parentData) {
    ...
    if (element.is('Classifier')) {
        ...
        const createTypeInfo = async (currType) => {
            ...
            if (!currTypeInfo) {
                currTypeInfo = new TypeInfo(currType.name);
            }
        ...
        const createProperty = (propertyInfo, me) => {
            me.typeInfo = propertyInfo.typeInfo;
        ...
        if (element.is('Stereotype')) {
            ...
            return function(applyingElement, manager) {
                if (applyingElement) {
                    ...
                    typeInfo.setBase(applyingElement.typeInfo);

...
export async function generate(element, umlClient, parentData) {
    ...
    if (element.isSubClassOf('classifier')) {
            ...
            for await (const property of front.attributes) {
                ...
                if (property.type.has()) {
                    ...
                    if (propertyType.id === STRING_ID) {
                        ...
                        if (property.defaultValue.has()) {
                            ...
                            defaultValueInfo.defaultValue = defaultValue.value;
                        }
                        const propertyInfo = {
                            name: property.name,
                            type: 'string',
                        };
                        properties.unshift({...propertyInfo, ...defaultValueInfo});
                        continue;
                ...
                if (multiplicity.lowerValue !== undefined && multiplicity.upperValue !== undefined) {
                    ...
                    if (multiplicity.lowerValue === 0 && multiplicity.upperValue === 1) {
                        const propertySingleton = {
                            name: property.name,
                            type: 'singleton'
                        };
                        properties.unshift({ ...propertySingleton, ...multiplicity});
                    } else {
                        const propertySet = {
                            name: property.name,
                            type: 'set',
                        };
                        properties.unshift({...propertySet, ...multiplicity});
                    }
                } else {
                    ...
                    properties.unshift(propertySet);
                }
            }
        }
        return function() {
            ...
            for (const propertyInfo of properties) {
                switch (propertyInfo.type) {
                    ...
                    case 'singleton': {
                        ...
                        // TODO subsets, redefines
                        this[propertyInfo.name] = propertySingleton;
                        this.sets[propertyInfo.name] = propertySingleton;
                        break;
                    }
                    default: 
        ...
        for await (const packagedElement of element.packagedElements) {
            ...
            if (packagedElement.isSubClassOf('package')) {
            ...
            } else if (packagedElement.isSubClassOf('classifier')) {
                const generatedElement = await generateElement();
                ...
                while (curr) {
                    ...
                    generationData.types[typeName] = generatedElement;
                    generationData.ids.set(element.id, generatedElement);
        ...
        ret[element.name + 'Manager'] = function(apiLocation) {
            ...
            this.post = function(type) {
                const typeClassJS = generationData.types[type];
                if (!typeClassJS) {
                    throw Error('Manager does not know type ' + type + ' to instantiate!');
                }
                const ret = new typeClassJS();
                const instanceSpecification = umlClient.post('instanceSpecification', {id: ret.id});

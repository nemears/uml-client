...
export default function parse(data) {
    if (data.abstraction) {
        ...
        parseStructuralFeature(data, 'property', ret);
        parseFeature(data, 'property', ret);
        ...
        const ret = new Interface();
        parseElement(data, 'interface', ret);
        parseNamedElement(data, 'interface', ret);
        parsePackageableElement(data, 'interface', ret);
        parseClassifier(data, 'interface', ret);
        parseInterface(data, 'interface', ret);
        return ret;
    } else if (data.signal) {
        const ret = new Signal();
        parseElement(data, 'signal', ret);
        parseNamedElement(data, 'signal', ret);
        parsePackageableElement(data, 'signal', ret);
        parseClassifier(data, 'signal', ret);
        parseSignal(data, 'signal', ret);
        return ret;
    }
}
...
function parseDataType(data, alias, dataType) {
    ...
    if (data[alias].ownedOperations) {
        for (let id of data[alias].ownedOperations) {
            dataType.ownedOperations.add(id);
        }
    }
}
...
function parseProperty(data, alias, property) {
    ...
    parseSet(data, alias, property.subsettedProperties, 'subsettedProperties');
    parseSet(data, alias, property.redefinedProperties, 'redefinedProperties');
}
...
function parseInterface(data, alias, interfaceEl) {
    if (data[alias].ownedAttributes) {
        for (let id of data[alias].ownedAttributes) {
            interfaceEl.ownedAttributes.add(id);
        }
    }
    if (data[alias].ownedOperations) {
        for (let id of data[alias].ownedOperations) {
            interfaceEl.ownedOperations.add(id);
        }
    }
}

function parseSignal(data, alias, signal) {
    if (data[alias].ownedAttributes) {
        for (let id of data[alias].ownedAttributes) {
            signal.ownedAttributes.add(id);
        }
    }
}
...
function parseFeature(data, alias, feature) {
    ...
    parseSingleton(data, alias, feature.featuringClassifier, 'featuringClassifier');
}

...
import { nullID } from './types/element.js'
...
export async function internalSet(val, singleton) {
    ...
    if (me.val || me.subSetsWithData.size > 0) {
        let bottomSet = me;
        if (!bottomSet.val) {
            // subSetsWithData will max be size of 1 because of singleton
            bottomSet = me.subSetsWithData.values().next().value;
        }
        const currVal = bottomSet.val;
        if (currVal.el === undefined || currVal.el === null) {
            await restoreReference(currVal, bottomSet);
        }
        ...
        if (me.val && me.val.id === currVal.id) {
        ...
        // run opposite
        await oppositeRemove(bottomSet, currVal.el);
    }
    ...
    if (val) {
        ...
        if (typeof val === 'object') {
            if (!me.el.references.has(val.id)) {
                me.el.references.set(val.id, val);
            }
            // set up references
            ...
            if (!me.el.references.has(val)) {
                me.el.references.set(val, undefined);
            }
        }
    }
}
